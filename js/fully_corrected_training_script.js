...
function assignTeamMemberToCell(name, quarter, workstation) {
    ...
    // Ensure the team member exists, is active, and available in this quarter
    if (!tm || !tm.active || tm.unavailableQuarters.includes(quarter)) {
        alert(`${name} is not available in ${quarter}.`);
        return;
    }

    // Check if the team member is already assigned in this quarter
    if (isTeamMemberAssignedInQuarter(name, quarter)) {
        alert(`${name} is already assigned in ${quarter}.`);
        return;
    }

    // Validate "in-training" team member assignment
    if (tm.partialStations.includes(workstation)) {
        const hasFullySkilledTeammate = schedule[quarter][workstation].some(assignment => {
            let teammate = teamMembers.find(member => member.name === assignment.name);
            return teammate && teammate.stations.includes(workstation); // Fully skilled teammate
        });

        if (!hasFullySkilledTeammate) {
            alert(`${name} cannot be assigned to ${workstation} for training without a fully skilled team member.`);
            return;
        }
    }
    ...
    console.log(`Assigned ${name} to ${workstation} in ${quarter}.`);
}
 is not available in ${quarter}.`);
        return;
    }
