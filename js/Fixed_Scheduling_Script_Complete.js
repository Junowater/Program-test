...
// Enhanced Assign Workstations Function with Heuristics
    ...
    // Modified function to enforce "in training" constraint
    function assignWorkstationsEnhanced
    (index, teamMemberAssignments, startTime, maxExecutionTime) {
...
// Updated Script: Adding Constraints for "In Training" Assignments

// The existing script content is truncated for brevity.
...
function assignWorkstationsEnhanced(index, teamMemberAssignments, startTime, maxExecutionTime) {
    ...
    for (let qIdx = 0; qIdx < quarters.length; qIdx++) {
        ...
        for (let wsIdx = 0; wsIdx < workstations.length; wsIdx++) {
            ...
            // Require 2 team members if someone is training
            let requiredAssignments = isTraining ? 2 : 1;
    ...
    for (let qIdx = 0; qIdx < quarters.length; qIdx++) {
        ...
        for (let wsIdx = 0; wsIdx < workstations.length; wsIdx++) {
            ...
            if (isTraining) {
                // Ensure at least one fully skilled member is present
                let fullySkilledAssigned = existingAssignments.some(a => {
                    let assignedTm = teamMembers.find(tm => tm.name === a.name);
                    return assignedTm && assignedTm.stations.includes(workstation);
                });
                if (!fullySkilledAssigned) {
                    candidates = candidates.filter(tm => tm.stations.includes(workstation));
                }
            }
