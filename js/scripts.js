...
function assignTeamMemberToCell(name, quarter, workstation) {
    ...
    // Check if the team member is in training and if another skilled team member is assigned
    // Check if the team member is in training and if another skilled team member is assigned
    const isInTraining = teamMembers.find(tm => tm.name === name).partialStations.includes(workstation);
    if (isInTraining) {
        const hasSkilledPartner = schedule[quarter][workstation].some(assignment =>
            teamMembers.find(tm => tm.name === assignment.name).stations.includes(workstation)
        );
        if (!hasSkilledPartner) {
            console.warn(`${name} is in training and cannot be assigned to ${workstation} without a skilled team member.`);
            return; // Prevent assignment but do not interrupt the flow.
        }
    }
    ...
    // Log the current state of schedule before assignment
    console.log("Before assignment:", JSON.stringify(schedule, null, 2));

    // Check if the team member is in training and if another skilled team member is assigned
    if (teamMembers.find(tm => tm.name === name).partialStations.includes(workstation)) {
        const hasSkilledPartner = schedule[quarter][workstation].some(assignment =>
            teamMembers.find(tm => tm.name === assignment.name).stations.includes(workstation)
        );
        if (!hasSkilledPartner) {
            console.warn(`${name} is in training and requires a skilled team member at ${workstation}.`);
            return; // Prevent assignment but do not interrupt the flow.
        }
    }

    // Ensure schedule has a valid entry for this quarter and workstation
    schedule[quarter] = schedule[quarter] || {};
    schedule[quarter][workstation] = schedule[quarter][workstation] || [];
    schedule[quarter][workstation].push({ name: name, lockState: 'none' });

    // Log the updated state of schedule after assignment
    console.log("After assignment:", JSON.stringify(schedule, null, 2));
...
// -------------------------------------------------------
// NEW FEATURE: Delete Last Date Saved from Weekly Chart
// -------------------------------------------------------
...
function addWorkstation(workstationName) {
    if (!workstations.includes(workstationName)) {
        ...
        // ADDED: Save immediately so we don't lose it on page reload
        saveData(false);
    } else {
        console.log(`Workstation '${workstationName}' already exists.`);
    }
}
...
function removeWorkstation(workstationName) {
    ...
    if (index !== -1) {
        ...
        // ADDED: Save immediately so we don't lose changes
        saveData(false);
    } else {
        console.log(`Workstation '${workstationName}' does not exist.`);
    }
}
...
// Updated addWorkstation function
function addWorkstation(workstationName) {
    if (!workstations.includes(workstationName)) {
        workstations.push(workstationName);
        quarters.forEach(q => {
...
// Updated removeWorkstation function
function removeWorkstation(workstationName) {
    const index = workstations.indexOf(workstationName);
    if (index !== -1) {
        workstations.splice(index, 1);
...
// Updated generateSkillsTable function
function generateSkillsTable() {
    const table = document.getElementById("skillsTable");
    if (!table) {
        console.error("Skills table element not found.");
